...
func (sc *storageContext) getGenerationParams(data *framework.FieldData) (exported bool, format string, role *roleEntry, errorResp *logical.Response) {
...
func generateCABundle(sc *storageContext, input *inputBundle, data *certutil.CreationBundle, randomSource io.Reader) (*certutil.ParsedCertBundle, error) {
	...
	if existingKeyRequested(input) {
		...
		keyEntry, err := getExistingKeyFromRef(sc, keyRef)
		if err != nil {
			return nil, err
		}
...
func generateCSRBundle(sc *storageContext, input *inputBundle, data *certutil.CreationBundle, addBasicConstraints bool, randomSource io.Reader) (*certutil.ParsedCSRBundle, error) {
	...
	if existingKeyRequested(input) {
		...
		key, err := getExistingKeyFromRef(sc, keyRef)
		if err != nil {
			return nil, err
		}
...
func getExistingKeyFromRef(sc *storageContext, keyRef string) (*keyEntry, error) {
